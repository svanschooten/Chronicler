...
class SettingsView(ft.Column):
    def __init__(
        ...
        on_llm_change: Callable[[], Awaitable[ModelCheck]] | None = None,
    ):
        ...
        self.llm_status = ft.Text("", size=12, color=self.colors.muted)
        self.llm_fields = ft.Column(spacing=16)
    ...
    @property
    def is_thin_client(self) -> bool:
        """Whether the work this page configures happens on a server instead."""
    ...
    def _connection(self) -> list[ft.Control]:
        ...
        if self.is_thin_client:
            controls.append(
                ft.Text(t("settings.connection.server_side"), size=12, color=self.colors.muted)
            )
        return controls
    ...
    def _llm(self) -> list[ft.Control]:
        """
        Nothing to configure here in thin-client mode.

        Summaries are generated by the server, from the server's own model
        configuration - an endpoint typed on this machine would be saved and then never
        read. The connection section says so instead. See docs/deployment-and-rpc.md.
        """
        if self.is_thin_client:
            return []

        self.llm_fields.controls = self._llm_fields()
        return [
            ...
            self.setting_card(
                ...
                self._dropdown(
                    ...
                    on_change=self._provider_changed,
                ),
            ),
            self.llm_fields,
            ft.Row(
                ...
                controls=[
                    ft.Container(expand=True, content=self.llm_status),
                    ft.TextButton(t("settings.llm.test"), on_click=self.test_llm_clicked),
                ],
            ),
        ]

    def _llm_fields(self) -> list[ft.Control]:
        """
        Only the fields the chosen provider actually reads.

        A gateway has no .gguf file and a local model has no API key, and showing both
        sets at once is how "model" came to look like a second required field rather
        than the default it is.
        """
        if self.settings.llm.provider == "llama_cpp":
            return [
                self.setting_card(
                    t("settings.llm.model_path"),
                    t("settings.llm.model_path_description"),
                    self._text_field("llm.model_path", width=260),
                )
            ]
        ...
        return [
            ...
            self.setting_card(
                ...
                t("settings.llm.api_key_description"),
                self._text_field("llm.api_key", width=260, secret=True),
            ),
            self.setting_card(
                t("settings.llm.model"),
                t("settings.llm.model_description"),
                self._text_field("llm.model", width=260),
            ),
        ]
    ...
    async def apply(self, path: str, value) -> bool:
        ...
        if path.startswith("llm."):
            await self._llm_reconfigured()
        return True

    async def _provider_changed(self, e):
        """Swapping providers changes which fields are worth showing at all."""
        await self._value_changed(e)
        self.llm_fields.controls = self._llm_fields()
        self._refresh(self.llm_fields)

    async def test_llm_clicked(self, e):
        self.llm_status.value = t("settings.llm.testing")
        self._refresh(self.llm_status)
        await self._llm_reconfigured()

    async def _llm_reconfigured(self) -> None:
        """
        Asks the app to re-read what the service layer can do, now that this changed.

        Without it the model configuration only took effect on the next launch: the
        worker reads the live settings, but the capabilities the views are built from
        were read once at startup, so summaries stayed greyed out. See docs/desktop.md.
        """
        if self.on_llm_change is None:
            return
        self._show_llm_status(await self.on_llm_change())

    def _show_llm_status(self, check: ModelCheck) -> None:
        if self.settings.llm.provider == "none":
            self.llm_status.value = ""
        elif check.error:
            self.llm_status.value = t("settings.llm.test_failed", error=check.error)
        elif check.models:
            self.llm_status.value = t(
                "settings.llm.test_ok", model=self.settings.llm.model or check.models[0]
            )
        else:
            self.llm_status.value = t("settings.llm.test_empty")
        self._refresh(self.llm_status)
